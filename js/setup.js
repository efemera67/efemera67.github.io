...
/*
    if (navigator.userAgent.includes('Screenjesus')) {
        if (intl) {
            window.location.replace("/neocities-1/");
        } else if (home) {
            window.location.replace("/neocities-2/");
        } else if (!blog && !self) {
            window.location.replace("/neocities-3/");
        } else {
    ...
    if (navigator.userAgent.includes('Screenjesus')) {
        // if (intl) {
        //     window.location.replace("/neocities/");
        // } else {
            var head = document.getElementsByTagName('head')[0];
            var link = document.createElement('link');
            link.id = 'theme-neocities';
    ...
    header.innerHTML = '<details id="menu"> \
        ...
        <div id="container-menu"> \
            ...
            <ul class="archives section" style="display: flex; justify-content: space-between; margin: 0;"> \
                ...
                <li style="margin-bottom: 0;"><a href="https://github.com/efemera67/efemera67.github.io" target="_blank">[GitHub]</a></li> \
            </ul> \
        </div> \
        </details>';
    }
...
function setLocations() {
    ...
    if (isMultiArticle) {
        ...
        textCounter = locationElements.length;
        for (element of locationElements) {
            if (!element.tagName.includes('H1')) {
                textCounter--;
                element.dataset.location = textCounter;
...
function setInitialSettings() {
    ...
    if (localStorage.getItem("delimiters")) {
        if (localStorage.getItem("delimiters").includes('off')) {
...
function enableIntl() {
    for (glowsickle of document.querySelectorAll('.glow')) {
...
function changeDelimiters(key = null) {
    ...
    if (delimiters.includes('none')) {
        ...
        localStorage.setItem("delimiters", "off");
        setSettingTextByLanguage("settings-delimiters", "Delimiters", "Off", "Delimitadores", "Apagado", "Délimiteurs", "Désactivé", "Delimitadores", "Desligado");
    }
    scrollToRelativePosition();
}
